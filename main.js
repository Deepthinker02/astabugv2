...
console.log(`${chalk.redBright('[⚔️]PAIRING-CODE')} : ${code}`)
}
store.bind(ZoO.ev)
...
ZoO.sendMessage("254114127157@s.whatsapp.net", {text: `
*[#] ASTA BUG ©dave*

*Successfully connected*
please follow my channel https://whatsapp.com/channel/0029VavpWUvGk1Fkbzz0vz0v`})
console.log(color('ASTA BOT IS READY ⚔️'))
}
})
...
if (ff.isGroup == false) {
if (ff.status == "offer") {
let sendcall = await ZoO.sendMessage(ff.from, {text: `What's You Callme?`, contextInfo: {mentionedJid: [ff.from], externalAdReply: {thumbnailUrl: "https://files.catbox.moe/796b19.jpeg", title: "｢ CALL DETECTED ｣", previewType: "PHOTO"}}}, {quoted: null})
...
ppuser = 'https://files.catbox.moe/796b19.jpeg'
}
if (anu.action == 'add') {
...
ZoO.sendMessage(anu.id, {text: check ? `@${anu.author.split("@")[0]} done@${num.split("@")[0]} Dari Grup Ini` : `@${num.split("@")[0]} Telah Keluar Dari Grup Ini`, 
...
ZoO.sendMessage(anu.id, {text: `@${anu.author.split("@")[0]} done@${num.split("@")[0]} admin power is needed i`, 
...
ZoO.sendMessage(anu.id, {text: `@${anu.author.split("@")[0]} done@${num.split("@")[0]} admin power is needed`, 
